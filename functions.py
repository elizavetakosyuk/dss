...
def create_obj_xml(name, in_age, in_friend):
    import xml.etree.ElementTree as ET
    animal = ET.Element('Animal')
    age = ET.SubElement(animal, 'Age')
    friend = ET.SubElement(animal, 'Friend')
    animal.set('name', name)
    age.text = in_age
    friend.text = in_friend
...
def add_new_obj_xml(xml_tree):
    import xml.etree.ElementTree as ET
    animals = xml_tree.getroot()

    # Записать в файл новые данные из консоли.
    print(f'Добавим новый элемент')
    name = input("Введите имя: ")  # 'cat Taychik'
    age = input("Введите возраст: ")  # '5months'
    friend = input("Введите имя друга: ")  # 'DogMichle'
    new_animal = create_obj_xml(name, age, friend)

    animals.append(new_animal)

    new_tree = ET.ElementTree(animals)
    return new_tree
